...
--- Boot orchestration: return chainable handle; :custom() or deferred auto :run()
--- 顺序：custom(immediately=true) → packages 配置 → custom(其余，最后)
--- Order: custom(immediately=true) → packages configs → remaining custom last
...
return function(config)
	...
	-- Auto-run after current script if boot() was not followed by :custom()
